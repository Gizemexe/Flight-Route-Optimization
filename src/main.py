...
def predict_and_plot(graph, path, total_distance, linear_model, scaler, alternative_paths=None):
    ...
    nx.draw_networkx_edges(subgraph, pos, edgelist=edges, edge_color='red', width=4, label="Optimal Path", alpha=0.7,
                           arrows=True)

    if alternative_paths:
        for alt_path in alternative_paths:
            alt_edges = list(zip(alt_path[:-1], alt_path[1:]))
            nx.draw_networkx_edges(subgraph, pos, edgelist=alt_edges, edge_color='green', width=3, alpha=0.7,
                                   label="Alternative Path", arrows=True)
...
if __name__ == "__main__":
    file_path = 'src/data/flights.csv'
    ...
    fastest_path, total_distance, total_delay, alternative_paths = find_and_display_fastest_path(graph, origin,
                                                                                                 destination)
